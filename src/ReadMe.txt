...
set to TRUE, so that emptyCarPark function waits until this is false. So when waiting car is parked this becomes false.
...
- Run the 'CarSimulation' instance by clicking the run button.
- Choose total number of spaces available in car park e.g. 50. Click enter.
- Choose total number of cars that need parking e.g. 100. Click enter.
- Watch the program park cars for a random amount of time until all spaces are full.
- A queue will form of cars wanting to park.
- Cars that have finished parking will leave at random times and make a space available.
- The queue of cars will let the first one into the new space.
- The program ends when all cars have finished parking and left the car park.
- Message 'Car Park has become empty' will display on console.
